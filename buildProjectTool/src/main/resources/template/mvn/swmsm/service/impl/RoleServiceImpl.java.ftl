...
import ${basePackageName}.${projectName}.core.dao.system.RoleDao;
...
import ${basePackageName}.${projectName}.core.entity.RoleMenuDO;
...
<RoleDO> list() {
    List
    <RoleDO> roles = roleMapper.list(new HashMap<>(16));
        return roles;
        }
        ...
        <RoleDO> list(Long userId) {
            List
            <Long> rolesIds = userRoleMapper.listRoleId(userId);
                List
                <RoleDO> roles = roleMapper.list(new HashMap<>(16));
